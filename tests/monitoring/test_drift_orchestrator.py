...
def test_load_global_state_uses_json_authority(tmp_path):
    status_file = tmp_path / "drift" / "rolling_90d_status.json"
    confidence_state_file = tmp_path / "drift" / "confidence_drift_state.json"
        ...
        json.dumps(
            {
                ...
                "status": "STOP",
                "alerts": ["CALIBRATION_DRIFT: 0.480 (Baseline 0.15)"],
                "metrics": {"ece": 0.48},
            }
        ),
        encoding="utf-8",
...
def test_append_drift_alerts_writes_authoritative_csv(tmp_path, monkeypatch):
